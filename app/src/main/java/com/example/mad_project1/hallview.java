...
public class hallview extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hallview);


        vievfliper1 = findViewById(R.id.blueRay);
        int image[] = {R.drawable.blueone, R.drawable.blutwo};
        for(int i = 0; i < image.length; i++){
            showSlider1(image[i]);
        }

        vievfliper2 = findViewById(R.id.redview);
        int image2[] = {R.drawable.redone, R.drawable.redtwo};
        for(int i = 0; i < image.length; i++){
            showSlider2(image2[i]);
        }

        vievfliper3 = findViewById(R.id.royal);
        int image3[] = {R.drawable.luxone, R.drawable.luxtwo};
        for(int i = 0; i < image.length; i++){
